...
const CACHE_KEY = 'jobhunt_ai_explain_cache_v1'
...
function fingerprint(q) {
    // Stable key: question text + correct answer index
    const txt = (q.question || '').trim().slice(0, 120)
    const ca = typeof q.correctAnswer === 'number' ? q.correctAnswer : -1
    return `mcq|${txt}|${ca}`
}
...
export default function AIExplainer({ question }) {
    const fp = useMemo(() => fingerprint(question), [question])
